...
public class ServerTCP {
	...
	private static class ServerResponse {
		...
		public static JSONObject quantityRequest(String clientName) {
			...
			json.put("data", "Server->Hello " + clientName + ", how many characters would you like to attempt to answer.");
			return json;
		}
		...
		public static JSONObject restartGame() {
			JSONObject json = new JSONObject();
			json.put("datatype", "yes or no");
			json.put("data", "Server->Would you like to play again? (yes/no)");
			return json;
		}
		...
		try {
			...
			try {
				...
				while (exit == false) {
							...
							switch (sequence) {
								...
								case (2) -> {
									try {
										...
										jsonToClient = ServerResponse.error("Invalid Request", 2);
									}
								}
								// Ready
								case (3) -> {
					...
					while (game.hasRemainingTime() && !win) {
						...
						for (int i = 0; i < picturePath.size(); i++) {
							...
							if (game.win()) {
								...
								NetworkUtility.Send(out, JsonUtility.toByteArray(ServerResponse.restartGame()));
								byte[] bytesFromClient = NetworkUtility.Receive(in);
								jsonFromClient = JsonUtility.fromByteArray(bytesFromClient);
								String restart = jsonFromClient.getString("data");
								if (!restart.equalsIgnoreCase("no") && !restart.equalsIgnoreCase("yes")) {
									jsonToClient = ServerTCP.ServerResponse.error("Would you like to play again? (yes/no)", 2);
